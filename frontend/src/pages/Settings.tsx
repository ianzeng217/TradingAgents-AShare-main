...
export default function Settings() {
    ...
    const handleDeleteToken = async (tokenId: string) => {
        if (!confirm('确认吊销该凭证？吊销后对应的 API 请求将立即失效，操作不可恢复。')) return
        try {
    ...
    return (
        <div className="space-y-6">
            <div>
                <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100">系统参数配置</h1>
                <p className="text-slate-500 dark:text-slate-400 mt-1">管理模型接入、分析参数与 API 访问权限</p>
            </div>
                    ...
                    <div className="md:col-span-2">
                        ...
                        <div className="relative">
                            ...
                            <input
                                ...
                                placeholder={hasStoredApiKey ? '已保存，如需更换请重新填写' : '输入模型服务商的 API Key'}
                                disabled={configLoading}
                            />
                        </div>
                        <div className="mt-2 flex flex-wrap items-center justify-between gap-3">
                            <div className="text-xs text-slate-500 dark:text-slate-400">
                                {serverFallbackEnabled
                                    ? '后端已启用公共模型兜底：未配置个人密钥时，将回退至服务端默认模型。'
                                    : '后端已关闭公共模型兜底：未配置个人密钥时，无法发起需要模型调用的分析任务。'}
                            </div>
                            {hasStoredApiKey && (
                                <button
                                    type="button"
                        ...
                        <p className="mt-2 text-xs text-slate-500 dark:text-slate-400">
                            保存后系统将自动验证连通性；也可点击下方"发起测试"，手动确认模型是否正常响应。
                        </p>
                    </div>
            ...
            <div className="card space-y-4">
                <div className="flex items-center gap-2">
                    ...
                    <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100">默认分析参数</h2>
                </div>
                ...
                <div>
                    ...
                    <textarea
                        ...
                        placeholder="例如：重点关注估值安全边际、政策催化与机构资金行为，偏好中长期价值研判。"
                    />
                </div>
            </div>
            ...
            <div className="card space-y-4">
                <div className="flex items-center gap-2">
                    ...
                    <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100">API 访问凭证</h2>
                    {tokensLoading && <Loader2 className="w-4 h-4 animate-spin text-slate-400 ml-auto" />}
                </div>

                <div className="text-sm text-slate-500 dark:text-slate-400 mb-4">
                    用于在第三方应用中调用本平台投研分析接口，请妥善保管，切勿对外泄露。
                </div>
                ...
                {newlyCreatedToken && (
                    <div className="p-3 rounded-2xl bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800">
                        <div className="text-sm font-medium text-emerald-800 dark:text-emerald-200 mb-1">访问凭证已生成 — 请立即复制并妥善保存，关闭后将无法再次查看</div>
                        <div className="flex items-center gap-2">
                        ...
                        <button onClick={() => setNewlyCreatedToken(null)} className="mt-2 text-xs text-emerald-600 hover:underline">已复制，关闭提示</button>
                    </div>
                )}
                ...
                {tokens.length >= 10 && (
                    <p className="text-[10px] text-amber-500">已达创建上限（最多 10 个）</p>
                )}
            </div>
            ...
            <div className="card space-y-4">
                <div className="flex items-center gap-2">
                    ...
                    <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100">研报推送配置</h2>
                </div>
                ...
                <div className="rounded-xl border border-slate-200/80 bg-slate-50/80 px-4 py-3 space-y-3 dark:border-slate-700/80 dark:bg-slate-900/40">
                    <div className="flex items-center justify-between">
                        <div>
                            <div className="text-sm font-medium text-slate-700 dark:text-slate-200">企业微信机器人</div>
                            <div className="text-xs text-slate-400 dark:text-slate-500 mt-0.5">
                                定时投研完成后，向企业微信机器人推送研报摘要
                    ...
                    <div className="flex items-center gap-2">
                        <div className="relative flex-1">
                            ...
                            <input
                                ...
                                className="input w-full pl-10"
                                placeholder={hasStoredWebhook ? '已保存，如需更换请重新填写' : 'Webhook URL'}
                                disabled={configLoading}
                            />
                        </div>
                        <button
