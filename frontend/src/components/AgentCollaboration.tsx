...
function GroupLabelNode({ data }: NodeProps<GroupLabelFlowNode>) {
    return (
        <div
            className="rounded-lg border border-slate-200/60 dark:border-slate-700/40 bg-slate-50/30 dark:bg-slate-800/10 pointer-events-none"
            style={{ width: data.width, height: data.height }}
        >
            <div className="absolute -top-2.5 left-3 px-1.5 bg-white dark:bg-[#0d1526]">
                <span className="text-[9px] font-bold text-slate-400 dark:text-slate-600 uppercase tracking-[0.18em]">
                    {data.label}
                </span>
            </div>
        </div>
                ...
                {isAnalyzing && (
                    <div className="flex items-center gap-4">
                        ...
                        <div className="text-right">
                            ...
                            <p className="fin-label mt-0.5">分析进度</p>
                        </div>
                    </div>
                )}
            </div>
